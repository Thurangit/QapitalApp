...
const EpargneAPropos = memo((props) => {
  ...
  const handleChange = (event) => {
    ...
    setInputs((values) => ({ ...values, [name]: value, userid, idepargne }));
  };
  ...
  const fetchAllcount_members_cotisation = () => {
    http
      .get("count/members/cotisation/" + userid + "/" + idepargne)
      .then((res) => {
  ...
  const fetchAllmembers_association = () => {
    http.get('get/members/association/' + userid + '/' + idepargne).then(res => {
  ...
  const fetchAlladmins_association = () => {
    http.get('get/admins/association/' + userid + '/' + idepargne).then(res => {
  ...
  const fetchAllepargne_info = () => {
    http
      ...
      .then((res) => {
        ...
        setInputs({
          nom: res.data.name_epargne_association,
          description: res.data.description_epargne_association,
        });
      });
  };
  ...
  const submitForm = () => {
    setIsLoading(true);
    http.post("/update/info/epargne", inputs).then((res) => {
      fetchAllepargne_info();
      handleClose1()
      setIsLoading(false);
    });
  ...
  const fetchAllinfo_user = () => {
    http.get('infos/user/' + epargne_info.creator_epargne_association).then(res => {
        ...
        <Col lg="12">
          <Card>
            ...
            <Card.Body>
              ...
              <div className="mt-2">
                <div className="mb-1 normaltext">Nom: <Link className="ms-3">{epargne_info.name_epargne_association}</Link></div>
                <div className="mb-1 normaltext">objectif: <Link className="ms-3">{epargne_info.montant_total_epargne_association} XAF</Link></div>
                ...
                <div className="mb-1 normaltext">Date de création: <Link className="ms-3">{new Date(epargne_info.created_at).toLocaleString()}</Link></div>
                <div className="mb-1 normaltext">Description: <p className="ms-3">{epargne_info.description_epargne_association}</p></div>
              </div>
            </Card.Body>
          </Card>
        </Col>
